...
class GradualEncoderUnfreezingTests(unittest.TestCase):
    def test_plateau_unfreezes_one_encoder_stage_after_ten_epochs(self):
        ...
        for _ in range(9):
            depth, plateau, changed = advance_encoder_unfreezing(
                depth, plateau, improved=False, patience=10
            )
    ...
    def test_plateau_scheduler_monitors_iou_with_patience_five(self):
        parameter = torch.nn.Parameter(torch.ones(1))
        optimizer = torch.optim.AdamW([parameter], lr=1e-4)
        scheduler = create_plateau_scheduler(optimizer, min_lr=1e-6)
