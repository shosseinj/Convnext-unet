...
class CrossScaleAttentionFusionV2(nn.Module):
    """Adjacent-scale normalized fusion with a per-channel residual gate."""

    def __init__(
        self, source_channels, source_indices, target_index, target_channels
    ):
        super().__init__()
        hidden_channels = max(16, target_channels // 4)
        ...
        self.projections = nn.ModuleList(
            nn.Sequential(
                nn.Conv2d(source_channels[index], hidden_channels, 1, bias=False),
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(
                hidden_channels * len(self.source_indices),
                target_channels,
                1,
                bias=False,
            ),
            nn.GroupNorm(1, target_channels),
            nn.GELU(),
        )
        attention_channels = max(8, target_channels // 8)
        self.channel_attention = nn.Sequential(
    ...
    def forward(self, features):
        ...
        target_size = target.shape[-2:]
        projected = []
        for index, projection in zip(self.source_indices, self.projections):
            feature = projection(features[index])
            if feature.shape[-2:] != target_size:
                feature = F.interpolate(
                    feature, size=target_size, mode="bilinear", align_corners=False
                )
