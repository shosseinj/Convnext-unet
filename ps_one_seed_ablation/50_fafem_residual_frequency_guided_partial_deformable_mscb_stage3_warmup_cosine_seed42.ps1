...
$experiment = "one_seed_50_fafem_residual_frequency_guided_partial_deformable_mscb_stage3_warmup_cosine"
$outputName = "50_fafem_residual_frequency_guided_partial_deformable_mscb_stage3_warmup_cosine"
...
try {
    & (Join-Path $PSScriptRoot "Invoke-OneSeed-Ablation.ps1") `
        ...
        -DecoderWarmupEpochs 0 -UnfreezeSchedule none -LrScheduler warmup_cosine `
        -LrWarmupEpochs 5 -LearningRate 3e-4 -MinimumLearningRate 1e-6 `
        ...
        -EnableResidualFGMSCBLiteStage3 $false `
        -EnableDeformableResidualFGMSCBLiteStage3 $false `
        -EnableResidualFGMSCBAllSkips $false `
        -EnablePartialDeformableResidualFGMSCBLiteStage3 $true `
        -ResidualFGMSCBGuidanceInitStd 0.0 -ResidualFGMSCBSignedStrength $false `
        ...
        -UncertaintyRefinementVersion none -DryRun:$DryRun
