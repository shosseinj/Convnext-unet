param(
    ...
    [ValidateSet("plateau", "cosine_warm_restarts", "warmup_cosine", "two_stage_warmup_cosine_restart")][string] $LrScheduler = "plateau",
    [int] $LrWarmupEpochs = 12,
    [int] $CosineT0 = 8,
    [int] $CosineTMult = 2,
    [int] $RestartEpoch = 120,
    [double] $FirstCycleMinFactor = 0.1,
    [double] $RestartFactor = (1.0 / 3.0),
...
$trainCommand = @($python, (Join-Path $repoRoot "main_torch.py"),
    ...
    "--restart_epoch", [string]$RestartEpoch,
    "--first_cycle_min_factor", [string]$FirstCycleMinFactor,
    "--restart_factor", [string]$RestartFactor,
